	...
	public List<ImageDTO> setPathAndUploadFile(int use, List<MultipartFile> fileList) throws IOException {
		...
		if (use == FOR_SHOW) {
			...
			for (MultipartFile mf : fileList) {
				...
				imageDTO.setUse(201);
				additoryPath = "\\resources\\image_furniture\\";
				...
				filenamesForArrange.add(/* fileList.get(i).getOriginalFilename() */ defaultPath + savedName);
			}
...
//			}
	...
	public void removeBackground(String filename) {
		...
		try {
			...
			String file_path = /* defaultPath + "\\resources\\image_furniture_arrange\\" + */ filename; // 배치용 이미지가 있는 경로
			System.out.println("받은 파일 : " + file_path);
			
			Response response;

			response = Request.post("https://api.remove.bg/v1.0/removebg").addHeader("X-Api-Key", "*")
					.body(MultipartEntityBuilder.create().addBinaryBody("image_file", new File(file_path))
							.addTextBody("size", "auto").build())
					.execute();

			System.out.println(defaultPath + "\\resources\\image_furniture_arrange\\" + "_com" + filename);
			response.saveContent(new File(defaultPath + "\\resources\\image_furniture_arrange\\" + "_com" + filename));
//			response.saveContent(new File("C:\\Users\\samsung\\Desktop\\Temp.JPG"));
