			...
			<ul>
				<li>
					<div class="leftSection">
						...
						<img class="room" src="${myRoomPath}" alt="">

						<form name="posform" id="posform" method="post">
							<input type="hidden" id="x" name="x"> <input
								type="hidden" id="y" name="y"> <input type="hidden"
								name="rFilepath" value="${myRoomPath}"> <input
								type="hidden" id="fFilepath" name="fFilepath">
						</form>
				...
				<li>
					<div class="rightSection">
						<ul>
							...
							<li>모델명 : ${productName}</li>
							<li>가구 정보 : ${productInfo.width} × ${productInfo.length} ×
								${productInfo.height} (cm)</li>
						</ul>
						<br>
						<hr width="90%">
						<br> <br>
						...
						<% ArrayList<Integer[]> pSize = (ArrayList<Integer[]>)request.getAttribute("pSize");%>
							...
							pageContext.setAttribute("pWidth", pSize.get(index)[0]); 
							System.out.println("너비는 : " + pSize.get(index)[0] + " 높이는 : " + pSize.get(index)[1]);
							pageContext.setAttribute("pHeight", pSize.get(index)[1]);%>
								<img class="image" src="${list.path}" width="${pWidth}px" height="${pHeight}px" onclick="make(${pWidth}, ${pHeight})">
							</c:forEach>
						</div>
						<br> <br>
						<hr width="90%">
