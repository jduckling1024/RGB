...
public class CartDAO {
   ...
   public List<Object[]> getList(String id) throws SQLException{
      ...
      while(res.next()) {
         ...
         cartDTO.setAmount(res.getInt(3));
   ...
   public void register(CartDTO cartDTO) throws SQLException {
      conn = dataSource.getConnection();
      sql = "INSERT INTO cart (`memberId`, `productId`, `amount`) VALUES (?, ?, ?)\r\n"
            + "ON DUPLICATE KEY UPDATE amount = amount + ?;";
      psmt = conn.prepareStatement(sql);
      psmt.setString(1, cartDTO.getMemberId());
      psmt.setInt(2, cartDTO.getProductId());
      psmt.setInt(3, cartDTO.getAmount());
      psmt.setInt(4, cartDTO.getAmount());
      psmt.executeUpdate();
   ...
   public void delete(String[] list, String memberId) throws SQLException {
      conn = dataSource.getConnection();
      sql = "DELETE FROM cart WHERE `memberId` = ? AND productId = ?";
