...
def binder(client_socket, addr):
    ...
    try:
        ...
        # 슬래시 몇 개로 나오는지 확인
         ...
         print('배치 가구 경로: '+furniture_filepath)
         print('배치 이미지: ' +furniture_filename)
         print('방 이미지 경로: '+room_filepath) # 이미지 byte로 받아와여 함
         
       
         source = cv2.imread(room_filepath)
         target = cv2.imread(furniture_filepath, cv2.IMREAD_UNCHANGED)
         ...
         img = mc.color_transfer(source, target, arranged_x, arranged_y)
         ...
         # cv2.imwrite('C:\\Users\\DeepLearning_7\\eclipse-workspace\\RoomGuidebook\\src\\main\\webapp\\'
